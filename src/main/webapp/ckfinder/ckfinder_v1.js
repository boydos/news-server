	...
	(function() {
		...
		CKFinder.prototype = {
			...
			CreateHtml : function() {
				...
				if (c && c.length > 0)
					c = ' id="' + c + '"';
				return '<iframe src="' + d._BuildUrl() + '" width="' + d.Width
						+ '" ' + 'height="' + d.Height + '"' + b + c
			...
			Popup : function(b, c) {
				...
				if (typeof b == 'string' && b.length > 1
					...
					b = parseInt(window.screen.width * parseInt(b, 10) / 100,
							10);
				if (typeof c == 'string' && c.length > 1
						&& c.substr(c.length - 1, 1) == '%')
					c = parseInt(window.screen.height * parseInt(c, 10) / 100,
							10);
				if (b < 200)
					b = 200;
				if (c < 200)
				...
				var d = parseInt((window.screen.height - c) / 2, 10), e = parseInt(
						...
						+ b + ',height=' + c + ',top=' + d + ',left=' + e, g = window
						.open('', 'CKFinderPopup', f, true);
				if (!g)
					return false;
			...
			_BuildUrl : function(b) {
				...
				if (e.DisableThumbnailSelection) {
					...
					if (e.SelectThumbnailFunctionData)
						c += '&amp;tdata='
								+ encodeURIComponent(e.SelectThumbnailFunctionData);
					else if (!e.SelectThumbnailFunction && e.SelectFunctionData)
						c += '&amp;tdata='
								+ encodeURIComponent(e.SelectFunctionData);
				}
				if (e.StartupPath) {
					c += c ? '&amp;' : '?';
					c += 'start='
		...
		CKFinder.SetupFCKeditor = function(b, c, d, e) {
				...
				for ( var h in c) {
					...
					if (h == 'Width') {
						...
						if (typeof i == 'string' && i.length > 1
							...
							i = parseInt(window.screen.width * parseInt(i, 10)
									/ 100, 10);
						...
						if (typeof j == 'string' && j.length > 1
							...
							j = parseInt(window.screen.height * parseInt(j, 10)
									/ 100, 10);
			...
			if (k.substr(0, 1) != '/' && k.indexOf('://') == -1)
				k = document.location.pathname.substring(0,
			...
			b.Config.LinkUploadURL = m + 'core/connector/'
					+ CKFinder.ConnectorLanguage + '/connector.'
					+ CKFinder.ConnectorLanguage
					+ '?command=QuickUpload&type=Files';
			b.Config.ImageUploadURL = m + 'core/connector/'
					+ CKFinder.ConnectorLanguage + '/connector.'
					+ CKFinder.ConnectorLanguage + '?command=QuickUpload&type='
					+ (d || 'Images');
			b.Config.FlashUploadURL = m + 'core/connector/'
					+ CKFinder.ConnectorLanguage + '/connector.'
					+ CKFinder.ConnectorLanguage + '?command=QuickUpload&type='
					+ (e || 'Flash');
		};
		CKFinder.SetupCKEditor = function(b, c, d, e) {
			...
			if (c !== null && typeof c == 'object') {
				...
				for ( var i in c) {
					...
					if (i == 'Width') {
						...
						if (typeof j == 'string' && j.length > 1
								&& j.substr(j.length - 1, 1) == '%')
							j = parseInt(window.screen.width * parseInt(j, 10)
									/ 100, 10);
						...
						if (typeof k == 'string' && k.length > 1
							...
							k = parseInt(window.screen.height * parseInt(k, 10)
									/ 100, 10);
			...
			if (l.substr(0, 1) != '/' && l.indexOf('://') == -1)
				l = document.location.pathname.substring(0,
						document.location.pathname.lastIndexOf('/') + 1)
						+ l;
			l = g._BuildUrl(l);
			var m = l.indexOf('?') !== -1 ? '&' : '?';
			...
			b.config.filebrowserImageBrowseUrl = l + m + 'type='
					+ (d || 'Images');
			b.config.filebrowserFlashBrowseUrl = l + m + 'type='
					+ (e || 'Flash');
			var n = l.substring(0, 1 + l.lastIndexOf('/'));
			b.config.filebrowserUploadUrl = n + 'core/connector/'
					+ CKFinder.ConnectorLanguage + '/connector.'
					+ CKFinder.ConnectorLanguage
					+ '?command=QuickUpload&type=Files';
