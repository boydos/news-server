...
TableList.prototype = {
	...
	firstInitTable : function () {
		...
		var table = null;
		if(!!this.headers && !!this.rows&& this.rows.length >0 ) {
			/*console.info("table ok");*/
...
/*			this.table.fnDraw();*/
	...
	create : function ( force ) {
		...
		console.info( "table- :" , !this.table, this.table);
		if( !this.table ) {
			this.firstInitTable();
		} else {
			this.reDrawTable();
	...
	getTableHeader : function ( table ) {
		...
		var len =this.headers.length;
		for(var i=0;i<len;i++){
       	 	headtr.append($(this.domhelper.getTH()).text(this.headers[i].name).attr('title', this.headers[i].comment));
   		}
